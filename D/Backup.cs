...
namespace D
{
    public class Backup
    {
        public const UInt64 U_RIGHT_MONITOR = 0x00000001;           // 实时监视 
        public const UInt64 U_RIGHT_PLAYBACK = 0x00000002;           // 录像回放 
        public const UInt64 U_RIGHT_DOWNLOAD = 0x00000004;           // 录像下载 
        public const UInt64 U_RIGHT_PTZ = 0x00000008;           // 云台控制 


        public static object EGTTotal(String CameraId,String nBeginStr = "2020-8-15 8:00:00",String nEndStr = "2020-8-16 8:00:00")
        {
            try
            {
                string szCameraId = CameraId;
                string strRet;
                uint totalCount;
                uint nQuerySeq;
                DateTime nBegin = Convert.ToDateTime(nBeginStr);        //开始时间
                DateTime nEnd = Convert.ToDateTime(nEndStr);          //结束时间
                UInt32 nStartTime = Convert.ToUInt32(nBegin.Subtract(Convert.ToDateTime("1970-1-1 8:00:00")).TotalSeconds);
                UInt32 nEndTime = Convert.ToUInt32(nEnd.Subtract(Convert.ToDateTime("1970-1-1 8:00:00")).TotalSeconds);
                int nGranularity = 2;
                IntPtr result = DPSDK_QueryPersonCount(nPDLLHandle, Encoding.Default.GetBytes(szCameraId), out nQuerySeq, out totalCount, nStartTime, nEndTime, nGranularity, (IntPtr)10000);
                if (result == IntPtr.Zero)
                    strRet = "查询总数成功，统计总数: %d" + Convert.ToString(totalCount);
                else
                    strRet = "统计总数失败，错误码：" + result.ToString();
        ...
        public String init()
        {
            ...
            //result = DPSDK_SetLog(nPDLLHandle, dpsdk_log_level_e.DPSDK_LOG_LEVEL_DEBUG, "D:\\DPSDK_LOG", false, false);
            result = DPSDK_SetLog(nPDLLHandle, dpsdk_log_level_e.DPSDK_LOG_LEVEL_DEBUG, strPath, false, false);
            //崩溃自动生成dmp文件
        ...
        public String Login(Login_Info_t loginInfo)
        {
            string Str = "登录成功";
            IntPtr result = DPSDK_Login(nPDLLHandle, ref loginInfo, (IntPtr)10000);
            if (result != (IntPtr)0)
            {
                Str = "登录失败,错误码：" + result.ToString();
            }
            return Str;
        }
      ...
      IntPtr pUserParam)
        {
            Console.WriteLine("报警类型：" + nAlarmType.ToString() + " 报警通道ID：" + szCameraId.ToString() + " 报警设备名称：" + szDeviceName.ToString() + " 报警通道名称：" + szChannelName.ToString() + " 报警信息：" + szMessage.ToString());

            //Alarm alarm = new Alarm();
            //alarm.type = nAlarmType.ToString();
            //alarm.CameraId = szCameraId.ToString();
            //alarm.device_name = szDeviceName.ToString();
            //alarm.ChannelName = szChannelName.ToString();
            //alarm.device_info = szMessage.ToString();

            //JObject rbc = JsonConvert.DeserializeObject<JObject>(JsonConvert.SerializeObject(alarm));

            //WebsocketServer.SetWebSocketMsg(rbc.ToString()); ;
            //strRet = "报警类型：" + nAlarmType.ToString() + " 报警通道ID：" + szCameraId.ToString() + " 报警设备名称：" + szDeviceName.ToString() + " 报警通道名称：" + szChannelName.ToString() + " 报警信息：" + szMessage.ToString();                
            return (IntPtr)0;
        }
        ...
        IntPtr DevStatusCallback(IntPtr nPDLLHandle,
                                 ...
                                 IntPtr pUserParam)
        {
            ...
            string str = " DeviceID:" + szDeviceId.ToString() + "  nStatus:" + status;
            strRet = str;
            return (IntPtr)0;
        }
        ...
        IntPtr NvrStatusCallback(IntPtr nPDLLHandle,
        ...
        IntPtr DPSDKTrafficAlarmCallback(IntPtr nPDLLHandle, ref Traffic_Alarm_Info_t pRetInfo, IntPtr pUserParam)
        {
            ...
            string szCarNum = pRetInfo.szCarNum;
            int nCarColor = pRetInfo.nCarColor;
            Pic_Url_t[] szPicUrl = new Pic_Url_t[6];
            for (int i = 0; i < 6; i++)
            {
                szPicUrl[i].szPicUrl = pRetInfo.szPicUrl[i].szPicUrl;
            }
            int nAlarmTime = pRetInfo.nAlarmTime;
            string szDeviceName = pRetInfo.szDeviceName;
            string szDeviceChnName = pRetInfo.szDeviceChnName;

            string str = "CameraId: " + szCameraId + "\r\nAlarmType: " + nAlarmType + "\r\nAlarmTime: " + nAlarmTime + "\r\nCarNum: " + szCarNum + "\r\nCarColor: " + nCarColor + "\r\nPictureURL0: " + szPicUrl[0].szPicUrl + "\r\nPictureURL1: " + szPicUrl[1].szPicUrl + "\r\nPictureURL2: " + szPicUrl[2].szPicUrl + "\r\nPictureURL3: " + szPicUrl[3].szPicUrl + "\r\nPictureURL4: " + szPicUrl[4].szPicUrl + "\r\nPictureURL5: " + szPicUrl[5].szPicUrl + "\r\nDeviceName: " + szDeviceName + "\r\nDeviceChnName: " + szDeviceChnName;
        ...
        IntPtr BayCarInfoCallback(IntPtr nPDLLHandle, ref Bay_Car_Info_t pRetInfo, IntPtr pUserParam)
        {
            string CameraID = pRetInfo.szChannelId;
            string DeviceName = pRetInfo.szDeviceName;
            string CannnelName = pRetInfo.szDeviceChnName;
            string CarNum = pRetInfo.szCarNum;
            string PicURL = pRetInfo.szImg0Path;

            string str = "CameraID:" + CameraID + "  CarNum:" + CarNum + "    PictureURL1:" + PicURL + "    PictureURL2:" + pRetInfo.szImg1Path + "    PictureURL3:" + pRetInfo.szImg2Path + "    PictureURL4:" + pRetInfo.szImg3Path + "    PictureURL5:" + pRetInfo.szImg4Path + "    PictureURL6:" + pRetInfo.szImg5Path + "    PlatePicURL:" + pRetInfo.szImgPlatePath;
        ...
        IntPtr DPSDKGetBayCarInfoCallbackEx(IntPtr nPDLLHandle,
                                            IntPtr szDeviceId,
                                            int nDeviceIdLen,
                                            int nDevChnId,
                                            IntPtr szChannelId,
                                            ...
                                            IntPtr pUserParam)
        {
            byte[] DeviceId = new byte[nDeviceIdLen];
            Marshal.Copy(szDeviceId, DeviceId, 0, nDeviceIdLen);
            string strDeviceId = Encoding.GetEncoding("utf-8").GetString(DeviceId);

            byte[] ChannelId = new byte[nChannelIdLen];
            Marshal.Copy(szChannelId, ChannelId, 0, nChannelIdLen);
            string strChannelId = Encoding.GetEncoding("utf-8").GetString(ChannelId);

            byte[] DeviceName = new byte[nDeviceNameLen];
            Marshal.Copy(szDeviceName, DeviceName, 0, nDeviceNameLen);
            string strDeviceName = Encoding.GetEncoding("utf-8").GetString(DeviceName);

            byte[] DeviceChnName = new byte[nChanNameLen];
            Marshal.Copy(szDeviceChnName, DeviceChnName, 0, nChanNameLen);
            string strDeviceChnName = Encoding.GetEncoding("utf-8").GetString(DeviceChnName);

            byte[] CarNum = new byte[nCarNumLen];
            Marshal.Copy(szCarNum, CarNum, 0, nCarNumLen);
            string strCarNum = Encoding.GetEncoding("utf-8").GetString(CarNum);

            byte[] CaptureOrg = new byte[nCaptureOrgLen];
            Marshal.Copy(szCaptureOrg, CaptureOrg, 0, nCaptureOrgLen);
            string strCaptureOrg = Encoding.GetEncoding("utf-8").GetString(CaptureOrg);

            byte[] OptOrg = new byte[nOptOrgLen];
            Marshal.Copy(szOptOrg, OptOrg, 0, nOptOrgLen);
            string strOptOrg = Encoding.GetEncoding("utf-8").GetString(OptOrg);

            byte[] OptUser = new byte[nOptUserLen];
            Marshal.Copy(szOptUser, OptUser, 0, nOptUserLen);
            string strOptUser = Encoding.GetEncoding("utf-8").GetString(OptUser);

            byte[] OptNote = new byte[nOptNoteLen];
            Marshal.Copy(szOptNote, OptNote, 0, nOptNoteLen);
            string strOptNote = Encoding.GetEncoding("utf-8").GetString(OptNote);

            byte[] Img0Path = new byte[nImg0PathLen];
            Marshal.Copy(szImg0Path, Img0Path, 0, nImg0PathLen);
            string strImg0Path = Encoding.GetEncoding("utf-8").GetString(Img0Path);

            byte[] Img1Path = new byte[nImg1PathLen];
            Marshal.Copy(szImg1Path, Img1Path, 0, nImg1PathLen);
            string strImg1Path = Encoding.GetEncoding("utf-8").GetString(Img1Path);

            byte[] Img2Path = new byte[nImg2PathLen];
            Marshal.Copy(szImg2Path, Img2Path, 0, nImg2PathLen);
            string strImg2Path = Encoding.GetEncoding("utf-8").GetString(Img2Path);

            byte[] Img3Path = new byte[nImg3PathLen];
            Marshal.Copy(szImg3Path, Img3Path, 0, nImg3PathLen);
            string strImg3Path = Encoding.GetEncoding("utf-8").GetString(Img3Path);

            byte[] Img4Path = new byte[nImg4PathLen];
            Marshal.Copy(szImg4Path, Img4Path, 0, nImg4PathLen);
            string strImg4Path = Encoding.GetEncoding("utf-8").GetString(Img4Path);

            byte[] Img5Path = new byte[nImg5PathLen];
            Marshal.Copy(szImg5Path, Img5Path, 0, nImg5PathLen);
            string strImg5Path = Encoding.GetEncoding("utf-8").GetString(Img5Path);

            byte[] ImgPlatePath = new byte[nImgPlatePathLen];
            Marshal.Copy(szImgPlatePath, ImgPlatePath, 0, nImgPlatePathLen);
            string strImgPlatePath = Encoding.GetEncoding("utf-8").GetString(ImgPlatePath);

            string str = "CameraID:" + strChannelId + "  DeviceName:" + strDeviceName + "  ChannelName:" + strDeviceChnName + "  CarNum:" + strCarNum + "    PictureURL1:" + strImg0Path + "    PictureURL2:" + strImg1Path + "    PictureURL3:" + strImg2Path + "    PictureURL4:" + strImg3Path + "    PictureURL5:" + strImg4Path + "    PictureURL6:" + strImg5Path + "    PlatePicURL:" + strImgPlatePath;
            // MessageBox.Show(str);
            strRet = str;
            return (IntPtr)0;
        }

        IntPtr GeneralJsonTransportCallbackEx(IntPtr nPDLLHandle,
                                              IntPtr ptrJson,
                                              int JsonLen,
                                              IntPtr pUserParam)
        {
            FileStream fs = new FileStream("D:\\Csharpjson.txt", FileMode.Create);
            byte[] json = new byte[JsonLen];
            Marshal.Copy(ptrJson, json, 0, JsonLen);
            fs.Write(json, 0, JsonLen);
            fs.Flush();
            fs.Close();
            //MessageBox.Show("json数据已经写入到D:\\Csharpjson.txt");
            return (IntPtr)0;
        }

        public IntPtr MediaDataCallback(IntPtr nPDLLHandle, IntPtr nSeq, int nMediaType, [MarshalAs(UnmanagedType.LPStr)] StringBuilder szNodeId, int nParamVal, IntPtr szData,
            int nDataLen, IntPtr pUserParam)
        {
            byte[] data = new byte[nDataLen];
            Marshal.Copy(szData, data, 0, nDataLen);
            Console.WriteLine(nSeq.ToString() + "_" + nDataLen.ToString());
            return (IntPtr)0;
        }


        IntPtr MediaDataFirstFrameCallback(IntPtr nPDLLHandle,
                                            int nSequence,
                                            IntPtr szCamearId,
                                            int nCameraIDLen,
                                            int nPlayMode,
                                            ...
                                            IntPtr pUserParam)
        {
            byte[] CamearId = new byte[nCameraIDLen];
            Marshal.Copy(szCamearId, CamearId, 0, nCameraIDLen);
            string strCamearId = Encoding.GetEncoding("utf-8").GetString(CamearId);

            //MessageBox.Show("第一帧码流回调，码流序号：" + nSequence.ToString() + " 通道ID:" + strCamearId + " 播放类型：" + nPlayMode.ToString() + " 码流厂商类型：" + nFactoryType.ToString());
            strRet = "第一帧码流回调，码流序号：" + nSequence.ToString() + " 通道ID:" + strCamearId + " 播放类型：" + nPlayMode.ToString() + " 码流厂商类型：" + nFactoryType.ToString();
            return (IntPtr)0;
        }
        ...
        ///@remark									
        IntPtr PecDoorStarusCallBack(IntPtr nPDLLHandle,
                    [MarshalAs(UnmanagedType.LPStr)] StringBuilder szCameraId,
                    dpsdk_door_status_e nStatus,
                    Int64 nTime,
                    IntPtr pUserParam)
        {
            //MessageBox.Show("通道ID：" + szCameraId.ToString() + " 门禁状态：" + nStatus.ToString() + " 上报时间：" + nTime.ToString());
            strRet = "通道ID：" + szCameraId.ToString() + " 门禁状态：" + nStatus.ToString() + " 上报时间：" + nTime.ToString();
            return (IntPtr)0;
        }
       ...
       */
        IntPtr VideoAlarmHostStatusCallback(IntPtr nPDLLHandle,
         [MarshalAs(UnmanagedType.LPStr)] StringBuilder szDeviceId, Int32 nChannelNO, Int32 nStatus, IntPtr pUserParam)
        {
            strRet = "VideoAlarmHostStatus DeviceId:" + szDeviceId.ToString() + " nChannelNO: " + nChannelNO.ToString() + " nStatus: " + nStatus.ToString();
            return (IntPtr)0;
        }
        ...
        */
        IntPtr NetAlarmHostStatusCallback(IntPtr nPDLLHandle,/* IN char* */
           [MarshalAs(UnmanagedType.LPStr)] StringBuilder szDeviceId, Int32 nRType, Int32 nOperType, Int32 nStatus, IntPtr pUserParam)
        {
            strRet = "NetAlarmHostStatus DeviceId:" + szDeviceId.ToString() + " nRType: " + nRType.ToString() + " nOperType：" + nOperType.ToString() + " nStatus: " + nStatus.ToString();
            return (IntPtr)0;
        }

        IntPtr InviteVtCallParamCallBack(IntPtr nPDLLHandle, ref InviteVtCallParam_t param, IntPtr pUserParam)
        {
            m_stuInviteVtCallParam.szUserId = param.szUserId;
            m_stuInviteVtCallParam.audioBit = param.audioBit;
            m_stuInviteVtCallParam.audioType = param.audioType;
            m_stuInviteVtCallParam.callId = param.callId;
            m_stuInviteVtCallParam.dlgId = param.dlgId;
            m_stuInviteVtCallParam.nCallType = param.nCallType;
            m_stuInviteVtCallParam.rtpAPort = param.rtpAPort;
            m_stuInviteVtCallParam.rtpServIP = param.rtpServIP;
            m_stuInviteVtCallParam.rtpVPort = param.rtpVPort;
            m_stuInviteVtCallParam.sampleRate = param.sampleRate;
            m_stuInviteVtCallParam.tid = param.tid;
            m_stuInviteVtCallParam.szCameraId = param.szCameraId;
            strRet += "收到呼叫邀请";
            bVtCallInvate = true;
            return (IntPtr)0;
        }
        /** 响铃通知回调
           ...
           @param	<IN>									pUserParam			用户指针参数,由对应的DPSDK_SetxxxxCallBack传入
       */
        IntPtr RingInfoCallBack(IntPtr nPDLLHandle, ref RingInfo_t param, IntPtr pUserParam)
        {
            strRet = "响铃通知";
            return (IntPtr)0;
        }
        /** 可视对讲呼叫状态繁忙通知回调
           ...
           @param	<IN>									pUserParam			用户指针参数,由对应的DPSDK_SetxxxxCallBack传入
       */
        IntPtr BusyVtCallCallBack(IntPtr nPDLLHandle, ref BusyVtCallInfo_t param, IntPtr pUserParam)
        {
            strRet = "呼叫繁忙";
            return (IntPtr)0;
        }
       ...
       */
        IntPtr AudioDataCallback([MarshalAs(UnmanagedType.LPStr)] StringBuilder pData, int dataLen, IntPtr pUserParam)
        {
            return (IntPtr)0;
        }
...
namespace H
{
    public class H
    {
        ...
        public static IntPtr realseq = default(IntPtr);
        public static int TvWallId = -1;
        public static int ScreenId = -1;
        public static int WindowId = -1;
        public static IntPtr playbackseq = default(IntPtr);
        public static int downloadSeq = 0;
        public static IntPtr m_pAudioDataFun;
        public static IntPtr m_pAudioDataUserParam;
       ...
       */
        ...
        */
        ...
        [DllImport("DPSDK_Core.dll", CharSet = CharSet.Ansi)]
        public extern static IntPtr DPSDK_SetDPSDKTrafficAlarmCallback(IntPtr nPDLLHandle, fDPSDKTrafficAlarmCallback pFun, IntPtr pUser);
       ...
       */
        ...
        */
        ...
        public struct Config_Emap_Addr_Info_t
        {
            ...
            [MarshalAs(UnmanagedType.LPStr)] StringBuilder szIP;                                    // 电子地图服务IP地址
        ...
        public struct OrgNodeS
        {
            public string parent;       // 父节点设备ID
            public string parentNmae;       // 父节点设备名称
            public string parentIP;       // 父节点设备IP
            public string parentPort;       // 父节点设备端口
            public string parentUser;       // 父节点设备账号
            public string parentPassword;       // 父节点设备密码
            public string devicePort; // 父节点设备端口

            public List<OrgNodeSC> c;//设备下通道
        }
        public struct OrgNodeSC
        {
            public string child;  // 子节点通道coding
            public string code;         // 本级节点编码
            public string name;         // 本级节点名称
            public string status;         // 本级节点状态
            public string channelType; //类型
        }
