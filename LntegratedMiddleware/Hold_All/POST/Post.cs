...
namespace LntegratedMiddleware.POST
{
    class Post
    {
        public static String HttpPostRaw(String Url, String jsonParas, out Boolean isSuccess)
        {
            Log.Debug(jsonParas);
            isSuccess = true;
        ...
        public static String HIKHttpRaw(String Url, String jsonParas, String TimeStr, String EncryptionKey, String AK, out Boolean isSuccess)
        {
            isSuccess = true;
            var client = new RestClient(Url);
            ...
            request.AddHeader("Date", TimeStr);
            request.AddHeader("X-Ca-Key", AK);
            request.AddHeader("X-Ca-Signature", EncryptionKey);
        ...
        public static String YSHttpRaw(string baseUrl, out Boolean error, string requestBody = null, String Authorization = null, Method Methods = Method.POST)
        {
            error = true;
            try
            {
                var client = new RestClient(baseUrl);
                var request = new RestRequest(Methods);
                request.AddHeader("Content-Type", "application/json");
                ...
                if (String.IsNullOrEmpty(response.Content))
                    error = false;
                //error = DataManipulation.VerifyReturnValue(JsonConvert.DeserializeObject<JObject>(response.Content));
