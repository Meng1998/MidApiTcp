...
namespace LntegratedMiddleware.zhongan.WebSocket
{
    ...
    public class WebsocketServer
    {
        ...
        public void WebSocketInit()
        {
            ...
            var ipstr = configuration["WebSocket:ip"];
            //server = new WebSocketServer($"ws://{ipv4[0]}:4649");//34.203.114.7
            server = new WebSocketServer($"ws://{ipstr}:4650");
            Log.Information("开始");
            try
            {
            ...
            catch (Exception)
            {
                ...
                while (Exceptionstate == new IntPtr(0))
                {
                    try
                    {
                        ...
                        Log.Debug("等待....");
                        server = new WebSocketServer($"ws://{ip}:4650");//34.203.114.7
                        server.Start(socket =>
                        {
                            socket.OnOpen = () =>
                            {
